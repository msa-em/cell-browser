...
Our attempt used the latent space of an autoencoder.
We replicated a recent method that used the [MNIST database](<wiki:MNIST_database>) and compared it to our model.
...
An image collection of over one million unique [cell nuclei](<wiki:Cell_nucleus>) was created to meet the scale demanded to train machine learning models.
...
It was impossible to justify this approach to image selection because the latent space is uninterpretable.
Machine learning models can be useful, but not if the task relies on knowing the internal workings of the model.
